...
dfi <- readRDS(here("~/science-ynn/regression_input_ynn.RDS"))  %>% 
  filter(year >= 2000)
...
standard_forms <- c(
  ...
  paste0("log_buildings_NOX", standard_controls, buildings_controls),
  paste0("log_electricity_heat_NOX", standard_controls, electricity_controls), 
  paste0("log_industry_NOX", standard_controls, industry_controls),
  paste0("log_transport_NOX", standard_controls, transport_controls))
...
standard_models %>% 
  saveRDS(here("~/science-ynn/main_model_result_parameter_variation1.RDS"))
##===============
