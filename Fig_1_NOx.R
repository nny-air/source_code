...
#buildings_color="#42d4f4"
#electricity_color="#3cb44b"
#industry_color = "#e6194b"
#transport_color = "#ffe119"
...
png("~/science-ynn/Figs/linechart.png", width     = 15,height    = 3,units     = "in",res       = 800)
linechart_cum_trend
dev.off()

pdf("~/science-ynn/Figs/linechart.pdf", width     = 15,height    = 3)
linechart_cum_trend
dev.off()
...
png("~/science-ynn/Figs/mix_plot_with_legend.png", width     = 15,height    = 6,units     = "in",res       = 800)
...
pdf("~/science-ynn/Figs/mix_plot_with_legend.pdf", width     = 15,height    = 6)
cowplot::plot_grid(plotlist = list(mix_final, legend_dev),nrow=2,rel_heights = c(1,0.1))+theme(plot.background = element_rect(colour = "grey", fill=NA, linewidth=3))
dev.off() 
...
#policy_out_filtered <- readRDS("~/science-ynn/Policy_out_filtered.RDS")
...
# 合并三个数据框
policy_out_filtered <- bind_rows(policy_out_filtered_NOX, policy_out_filtered_CO, policy_out_filtered_NMVOC) %>%
  group_by(country_sample, sector) %>%
  summarize(
    out = list(bind_rows(out)),          # 合并 `out` 列中的数据框
    is = list(unique(is)),               # 保留唯一的 `isatpanl` 值
    policy_match = list(bind_rows(policy_match)),           # 合并 `policy_match` 数据框
    policy_match_2y = list(bind_rows(policy_match_2y)),     # 合并 `policy_match_2y` 数据框
    policy_match_3y = list(bind_rows(policy_match_3y)),     # 合并 `policy_match_3y` 数据框
    .groups = "drop"                     # 去除分组
  )
...
png("~/Figs/Europe.png", width     = 10,height    = 10,units     = "in",res       = 800)
show(Europe)
dev.off() 

pdf("~/Figs/Europe.pdf", width     = 10,height    = 10)
show(Europe)
dev.off()

png("~/Figs/Asia_Oceania.png", width     = 10,height    = 10,units     = "in",res       = 800)
show(Asia_Oceania)
dev.off() 

pdf("~/Figs/Asia_Oceania.pdf", width     = 10,height    = 10)
show(Asia_Oceania)
dev.off() 

png("~/Figs/Americas.png", width     = 10,height    = 20,units     = "in",res       = 800)
show(Americas)
dev.off() 

pdf("~/Figs/Americas.pdf", width     = 10,height    = 20)
show(Americas)
dev.off() 

png("~/Figs/South_Africa.png", width     = 10,height    = 7,units     = "in",res       = 800)
show(South_Africa)
dev.off() 

pdf("~/Figs/South_Africa.pdf", width     = 10,height    = 7)
show(South_Africa)
dev.off()
...
for(c in unique(expanded_df$ISO)){
  ...
  path = paste("C:/Users/14329/Documents/science-ynn/Figs/Break_count_bar_charts",c,".png",sep="")
...
p_legend <- ggplot(expanded_df,aes(x=ISO,fill=sector))+
  ...
  scale_fill_manual(values = c(buildings_color,electricity_color,industry_color,transport_color),#Number of detected breaks in emissions of NOX, CO, and NMVOC combined between 2000 and 2022 by sector
                    labels = c('Buildings','Electricity','Industry','Transport'),name='Number of detected breaks in emissions\nof NOX CO and NMVOC combined\nbetween 2000 and 2022 by sector')+
...
png("~/science-ynn/Figs/legend_break_map2.png", width     = 20.00,height    = 5.00,units     = "in",res       = 800)
p
dev.off()
