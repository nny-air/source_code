...
input_data <- readRDS("~/science-ynn-CO/regression_input_ynn.RDS")
...
excl_all <- input_data %>% group_by(country) %>% 
  ...
  #summarise(excl_test = mean(logtotal_CO)) %>% filter(excl_test < 10) %>% pull(country) %>% unique
...
tot_core_deps <- c("log_buildings_CO",
                   "log_electricity_heat_CO", 
                   "log_industry_CO",
                   "log_transport_CO")
#原来的数据变量log_buildings_co2 改为现有的lbuild_CO，log_electricity_heat_co2改为lelec_heat_CO；log_industry_co2及log_transport_co2
...
eu_control_forms <- c(
  paste0("log_buildings_CO", buildings_controls),
  paste0("log_electricity_heat_CO", electricity_controls), 
  paste0("log_industry_CO", industry_controls),
  paste0("log_transport_CO", transport_controls))
...
saveRDS(results,"~/science-ynn-CO/Break_detection_results_ynn.RDS")
