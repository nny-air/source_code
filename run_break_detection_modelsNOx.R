...
library(foreach)
# Country groupings
#setwd("C:/Users/14329/Documents/science-ynn")
#Sets the working directory to where the country groupings and input data are stored.
##load country groups 
...
input_data <- readRDS("~/science-ynn/regression_input_ynn.RDS")
...
excl_all <- input_data %>% group_by(country) %>% 
  ...
  #summarise(excl_test = mean(logtotal_NOX)) %>% filter(excl_test < 10) %>% pull(country) %>% unique
...
samples <- mget(c("AC1", "AC6_2"))#将 AC1 和 AC6_2 合并为一个列表 samples，以便后续使用。
...
tot_core_deps <- c("log_buildings_NOX",
                   "log_electricity_heat_NOX", 
                   "log_industry_NOX",
                   "log_transport_NOX")
#原来的数据变量log_buildings_co2 改为现有的lbuild_NOX，log_electricity_heat_co2改为lelec_heat_NOX；log_industry_co2及log_transport_co2
...
eu_control_forms <- c(
  paste0("log_buildings_NOX", buildings_controls),
  paste0("log_electricity_heat_NOX", electricity_controls), 
  paste0("log_industry_NOX", industry_controls),
  paste0("log_transport_NOX", transport_controls))
...
#country:trend 是回归分析中用于描述国家和年份之间的交互作用的变量。这使得每个国家在不同年份的趋势是独立建模的
...
#run models with getspanel           #f = rel_forms_trends是一个包含回归公式的列表或向量。
...
results_developing <- foreach(f = rel_forms_trends, .combine = rbind, .packages = c('tidyverse', 'getspanel')) %:%
  ...
  foreach(p.value = c(0.01), .combine = rbind) %dopar% {
    dat <- df_restr_trends %>% filter(country %in% samples[[smpl]])##isatpanel是getspanel 包中的函数，用于估计面板数据回归模型
    is <- isatpanel(
      data = dat,#传递筛选后的数据 dat（即该国家组的相关数据）
      formula = as.formula(f),#f 是每次循环中从 rel_forms_trends 中获取的回归公式，as.formula(f) 将其转换为公式格式，以便传递给模型
      index = c("country", "year"),#指定面板数据的索引，这里是以 country 和 year 为索引，即每个国家在每一年的数据
      effect = "twoways",
      iis = ii,##这个参数控制是否包含个体内的异方差（ii），它在此设置为 FALSE，意味着没有考虑特定国家的时间趋势。这个变量将被存储到 iis 中。
      fesis = TRUE,###这个参数启用固定效应的异方差性处理。
      ar = 0,##设置为 0，表示不使用自回归模型，如果数据具有时间序列性质，可以通过设置适当的 ar 值来捕捉滞后效应
      t.pval = p.value,
      max.block.size = 20##最大块大小为 20，控制回归时每块数据的大小
    )#下面记录和保存回归模型的关键信息
...
saveRDS(results,"~/science-ynn/Break_detection_results_ynn.RDS")
