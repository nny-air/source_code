...
dfi <- readRDS(here("~/science-ynn-NMVOC/regression_input_ynn.RDS"))  %>% 
  filter(year >= 2000)
...
standard_forms <- c(
  ...
  paste0("log_buildings_NMVOC", standard_controls, buildings_controls),
  paste0("log_electricity_heat_NMVOC", standard_controls, electricity_controls), 
  paste0("log_industry_NMVOC", standard_controls, industry_controls),
  paste0("log_transport_NMVOC", standard_controls, transport_controls))
...
standard_models %>% 
  saveRDS(here("~/science-ynn-NMVOC/main_model_result_parameter_variation1.RDS"))
##===============
...
policy_out_NMVOC <- foreach(i = 1:nrow(results), .combine = rbind, .packages = c('tidyverse', 'getspanel')) %do% {
