...
oecd_grouped = read.csv('~/science-ynn-NMVOC/OECD_data_preprocessed.csv')
...
results = readRDS('~/science-ynn-NMVOC/Break_detection_results_ynn.RDS')
...
#[1] "The number of breaks in  Electricity  is: 17"
#[1] "The number of breaks in  Industry  is: 22"
#[1] "The number of breaks in  Transport  is: 21"

#translate into % (still needs to be multiplied with 100 of course)

all_together$coef_percent = exp(all_together$coef)-1

mean(all_together$coef_percent)#-0.2342813
...
saveRDS(policy_out,"~/science-ynn-NMVOC/Policy_out.RDS")
...
saveRDS(policy_out_filtered,"~/science-ynn-NMVOC/Policy_out_filtered.RDS")
...
# 指定输出文件路径（可以修改路径和文件名）
output_file <- "~/science-ynn-NMVOC/policy_match_2y_NMVOC.csv"
